...
public class Base64 {
	 ...
	 */
	public static void encode(
			java.nio.ByteBuffer raw, java.nio.CharBuffer encoded) {
		byte[] raw3 = new byte[3];
		byte[] enc4 = new byte[4];
	...
	public static String encodeBytes(
			byte[] source, int off, int len, int options)
			throws java.io.IOException {
	...
	public static byte[] encodeBytesToBytes(byte[] source) {
		...
		try {
			encoded =
					encodeBytesToBytes(source, 0, source.length,
							Base64.NO_OPTIONS);
		}
		catch (java.io.IOException ex) {
	 ...
	 * @since 2.3.1
	 */
	public static byte[] encodeBytesToBytes(
			byte[] source, int off, int len, int options)
			throws java.io.IOException {
			...
			if (sbiDecode >= WHITE_SPACE_ENC) {
				if (sbiDecode >= EQUALS_SIGN_ENC) {
					...
					if (b4Posn > 3) { // Time to decode?
						outBuffPosn +=
								decode4to3(b4, 0, outBuff, outBuffPosn, options);
						b4Posn = 0;
			...
			// the provided class loader.
				...
				ois = new java.io.ObjectInputStream(bais) {
					...
					public Class<?> resolveClass(
						...
						Class<?> c =
								Class.forName(streamClass.getName(), false,
										loader);
						if (c == null) {
	...
	public static void encodeToFile(byte[] dataToEncode, String filename)
		...
		try {
			bos =
					new Base64.OutputStream(new java.io.FileOutputStream(
	...
	public static String encodeFromFile(String filename)
		...
		try {
			...
			// Save in a variable to return
			encodedData =
					new String(buffer, 0, length, Base64.PREFERRED_ENCODING);
